...
Complete YouTube toolkit via [TranscriptAPI](https://transcriptapi.com). Transcripts, search, channels, and playlists in
one skill — no `yt-dlp`, no headless browser, no binary to keep patched.
...
**Output**: Calls the free `channel/latest` endpoint, returns new uploads with title, publish date, and view count — no
credits spent.

## Setup

First run prompts for a `TRANSCRIPT_API_KEY` (or Claude can provision one automatically). Free tier: 100 credits, no
card required.

## Tips

- `channel/latest` and `channel/resolve` are free — use them for polling and handle lookups before spending a credit on
  anything else
- Failed or rate-limited calls don't cost credits
- Works the same on a laptop or a cloud server — TranscriptAPI handles the infrastructure YouTube blocks for direct
  scraping
