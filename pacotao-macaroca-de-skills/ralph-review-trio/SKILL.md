...
This skill triggers `/ralph-review`, which runs three sequential reviewer subagents at increasing depth. If any tier
flags a failure, the loop restarts from Tier 1 after fixes.
...
- Documentation-only diffs with no code — Tier 2/3 still run but most checks short-circuit to "doc-only PR" exemption;
  overkill for a single README edit.
...
A `<promise>HAIKU_PASS</promise>` / `<promise>SONNET_PASS</promise>` / `<promise>OPUS_PASS</promise>` token is emitted
by each tier on pass. All three required for overall pass.
...
Extended reference content under `references/` (same dir as this SKILL.md) is loaded on demand by each tier when a
specific check requires more context.
