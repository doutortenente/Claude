...
Design and generate `.envelope.json` AI agent team definition files — the open standard for building and deploying
multi-agent teams on [Envelope](https://openenvelope.org).
...
**User**: "Build me an Envelope team for content moderation — a supervisor and two reviewer agents that check posts
...
Exactly one agent should have no `reportsToKey` — this is the supervisor that receives the initial task. All others
reference their manager's `key`.
...
- Use `anthropic:claude-sonnet-4-5` for supervisors and managers; `anthropic:claude-haiku-3-5` for leaf agents doing
  repetitive work
...
- [npmjs.com/package/@openenvelope/schema](https://www.npmjs.com/package/@openenvelope/schema) — TypeScript types +
  validator
