...
- **YouTube hosts only** — allowed hostnames include `youtube.com` (and subdomains such as `www`, `m`, `music`),
  `youtu.be`, and `youtube-nocookie.com` (including `www`). Other sites are rejected.
...
2. Otherwise it creates a **local virtualenv** at `video-downloader/.venv`, installs `yt-dlp` there with `pip`, and runs
   that binary.
3. If venv creation or install fails, follow the printed message or install globally, for example:
