...
2. **Abreviaturas padrão UTI**: `VM` = vent mecânica, `CN` = cateter nasal, `BH` = balanço, `DU` = diurese, `HD` = hipótese diagnóstica, `DH` = dia de hospitalização.
3. **Unidades implícitas**: `PAM 62` (mmHg), `Cr 1.8` (mg/dL), `Plaq 180` (×10³/µL). Só escreve unidade se fora do padrão.
