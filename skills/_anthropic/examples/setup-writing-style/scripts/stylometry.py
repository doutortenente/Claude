...
def load_register(reg_dir, is_chat, min_w, dropped):
    ...
    samples = []
    for parts in groups.values():  # insertion order == sorted file order
...
def main():
    ...
    for reg_dir in sorted(p for p in args.samples_dir.iterdir() if p.is_dir()):
        ...
        ex_lines.append(f"\n## Register: {reg}\n")
        for rank, i in enumerate(pick_exemplars(samples, args.per_register), 1):
            ...
            if len(body) > 1800:
                body = body[:1800] + " […]"
            tag = f" — bundle of {len(segs_i)} messages" if len(segs_i) > 1 else ""
