...
Configuração reutilizável do Claude Code do Dr. Tenente: skills vendor + settings (permissões, hooks). Fonte central
para sincronizar entre projetos.
...
Skills e settings daqui são copiados/symlinkados para `.claude/` de cada projeto. Não contém segredos. Workspace:
`~/projetos/` · índice em `memory/MAPA-DEV.md`.
