...
_CN_CHAPTER = re.compile(
    rf"^\s*第\s*([0-9{_FW_DIGITS}{_CN_NUM_CLASS}]+)\s*[章回卷节篇讲]"
)
_MD_CN_HEADING = re.compile(
    rf"^#{{1,6}}\s+第?\s*([{_FW_DIGITS}{_CN_NUM_CLASS}]+)\s*[·、.:：章回卷节篇讲]"
)
...
_TOC_HEADERS = (
    ...
    "indice",
    "sommario",  # Italian (no accent — distinct from índice above)
    "inhoudsopgave",  # Dutch
)
...
def extract_single_file(
    ...
    if ext == ".epub":
        ...
        if text is None or not text.strip():
            raise ExtractionError(
                f"Could not extract text from HTML: {input_path.name}"
            )
        method = "html-parser"
        pages = 0
        pages_label = "sections"
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: extract.py <path-to-document-folder-or-glob>... [--mode technical|text] [--install-missing ask|yes|no]",
            file=sys.stderr,
        )
        print(
    ...
    if not raw_input_paths:
        print(
            "ERROR: No input document, folder, or glob pattern specified.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not input_files:
        print(
            f"ERROR: No supported files found matching: {', '.join(raw_input_paths)}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    metadata = {
        ...
        "format": (
            "mixed" if len(extracted_sources) > 1 else extracted_sources[0]["format"]
        ),
        "extraction_method": (
            "multi-method"
            if len(extracted_sources) > 1
    ...
    print(
        f"   ToC     : {'yes' if consolidated_structure['has_toc'] else 'not detected'}"
    )
