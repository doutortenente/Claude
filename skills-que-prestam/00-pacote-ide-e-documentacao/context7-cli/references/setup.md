...
Without `--api-key` or `--oauth`, setup opens a browser for OAuth login. MCP mode additionally generates a new API key
after login. `--oauth` is MCP-only.

**What gets written — MCP mode:**

- MCP server entry in the agent's config file (`../../../../../.mcp.json` for Claude, `.cursor/mcp.json` for Cursor, `.opencode.json`
  for OpenCode)
...
- A `find-docs` skill in the chosen agent's skills directory, guiding the agent to use `ctx7 library` and `ctx7 docs`
  commands
