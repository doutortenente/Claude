...
Transform vague, ambiguous prompts into actionable, well-defined requests through systematic research and targeted
clarification. This skill is invoked when the hook has already determined a prompt needs enrichment.
...
- Route Glob, Grep, WebSearch, WebFetch, and multi-file Read through `Task/Explore` — never call them directly in main
  context
- Include conversation-relevant context (file paths, errors, prior decisions) in every Explore prompt — Explore cannot
  see prior turns
...
For question templates, effective patterns, and examples,
...
**Hook evaluation:** Determined prompt is vague **Original prompt:** "fix the bug"
...
For comprehensive examples showing various prompt types and transformations,
