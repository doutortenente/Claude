...
This reference provides real-world examples showing the complete flow from vague prompts through research, question
...
**Execution:**
Extract UserService class following ProductService pattern, move business logic from users.ts to service/UserService.ts,
...
**Execution:**
Implement JWT authentication with refresh tokens, support both HttpOnly cookies (web) and Authorization header (mobile),
create login/register endpoints with role-based access control.
...
**Execution:**
Implement Redis caching for product catalog and user profiles with 1-minute TTL, configure Redis connection, add cache
middleware to those endpoints.
...
These examples demonstrate the spectrum from clear to vague and show how research findings directly inform specific,
