...
def validate_rule(row, seen_ids=None):
    ...
    if has_action:
        ...
        if action_type not in legal:
            errors.append(f"action type {action_type!r} not legal for event {event!r}")
...
def load_rules():
    ...
    for path in sorted(NUDGES_DIR.glob("**/*.json")):
        ...
        if dir_event == NUDGES_DIR.name:
            print(
                f"[engine] skipping {path.name}: not in an event subdirectory",
                file=sys.stderr,
            )
            continue
        ...
        for row in rows:
            ...
            if errors:
                ...
                print(
                    f"[engine] skipping rule {rid}: {'; '.join(errors)}",
                    file=sys.stderr,
                )
                continue
            seen_ids.add(row["id"])
