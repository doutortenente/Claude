...
def calc_balanco(ganhos, perdas):
    ...
    def _consumir(itens, lado_informado):
        ...
        for it in itens or []:
            ...
            if (
                serie is not None
            ):  # células horárias cruas → o SCRIPT soma (não a enfermagem)
                ileg = sum(1 for v in serie if isinstance(v, str))
                cells = [float(v) for v in serie if isinstance(v, (int, float))]
                ml = sum(cells)
                if ileg:
                    warnings.append(
                        f"'{nome}': {ileg} célula(s) ilegível(is) — soma pode subestimar, revisar folha"
                    )
            else:
                ml = it.get("ml", None)
            if ml is None:
            ...
            if cat is None:
                warnings.append(
                    f"item '{nome}' não reconhecido no dicionário — NÃO somado (revisar)"
                )
                continue
...
def render_leito(leito, meta):
    ...
    conf = leito.get(
        "conferencia_enfermagem"
    )  # o que a enfermagem SOMOU à mão (não confiável)
    conf_linha = None
    if conf:
        divs = []
        for rot, calc, inf in [
            ("Ganhos", bal["ganho_total"], conf.get("ganhos")),
            ("Perdas", bal["perda_total"], conf.get("perdas")),
            ("BH", bal["balanco"], conf.get("bh")),
        ]:
            if inf is not None and abs(calc - inf) > 0.5:
                divs.append(
                    f"{rot}: folha diz {_fmt(inf)}, células somam {_fmt(calc)} ({calc-inf:+.0f})"
                )
        for d in divs:
    ...
    diurese = next(
        (
            ml
            for nm, ml in bal["itens_perda"]
            if classify(nm) == "perda" and "diur" in nm.lower()
        ),
        None,
    )
...
def build(payload):
    ...
    if meta:
        ...
        out.append(
            f"**Data:** {meta.get('data','?')} — plantão {meta.get('turno','?')}"
        )
        out.append(f"**Janela de sinais vitais:** {meta.get('janela','?')}")
        out.append(
            "**Doutrina:** zero alucinação · sinais vitais Max–Min · balanço = Σganhos − Σperdas (script)"
        )
        out.append("")
    for leito in leitos:
...
DEMO = {
    ...
    "leitos": [
        {
            "leito": "01",
            "iniciais": "Valdir L. C. (VLC)",
            "dia_internacao": "5º DIA",
            "sup_o2": "AA",
            "dieta": "geral hipossódica p/ diabético + Nutren Senior",
            "vitais": {
            ...
            "ganhos": [
                {"nome": "dieta+hidratação", "ml": 1030},
                {"nome": "diurese", "ml": 1100},
            ],
            "perdas": [],
            "evacuacao": "ausente no período",
            ...
            "impressao": [
                "IC aguda, estável — sem DVA.",
                "DRC agudizada / sd cardiorrenal, em melhora.",
            ],
            "conduta": [
                "CV/Renal: manejo IC + vigilância renal; meta PAM ≥ 65.",
                "Profilaxia: TEV/LAMG/cabeceira.",
            ],
        }
    ],
}
...
def main():
    ap = argparse.ArgumentParser(
        description="Motor determinístico da passagem de plantão UTI."
    )
    ...
    ap.add_argument(
        "--demo",
        action="store_true",
        help="roda caso de validação (inclui o bug clássico)",
    )
    a = ap.parse_args()
    if a.demo:
        payload = DEMO
