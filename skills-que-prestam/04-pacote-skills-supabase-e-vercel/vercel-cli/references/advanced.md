...
**Use `vercel api` when a CLI command doesn't exist for what you need.** Full access to the Vercel REST API with
...
Keep calls narrow and shape large responses before presenting them. Use `--raw-field` instead of typed `--field` when
the API expects a string value that looks like a boolean or number.
...
Register Vercel Apps (OAuth client IDs) and manage team installations. Useful for building integrations that
authenticate against a Vercel team.
...
`register` issues a client ID. `install` (alias `add`) installs an app to the current team using that client ID. At
least one `--permission` is required (the install errors with `Provide at least one --permission` otherwise); repeat
`--permission` for each scope the app needs.
