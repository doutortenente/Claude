...
Use these commands for infrastructure attached to a Vercel project: cache, cron jobs, deploy hooks, Git connections,
Edge Config, redirects, custom environments, and rolling releases.
...
Routing and redirect changes have separate command groups. Route changes are staged before `vercel routes publish`;
redirect changes use redirect versions and promotion.
...
`vercel target` lists custom environments. `deploy` and `build` accept `--target`; `pull` accepts `--environment` (not
`--target`).
