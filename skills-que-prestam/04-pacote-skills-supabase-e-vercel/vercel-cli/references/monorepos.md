...
Vercel auto-detects monorepo tools (Turborepo, Nx) and workspace managers (pnpm, Yarn, npm). Each project in the
monorepo gets its own Vercel project, linked via `vercel link --repo`.
...
Vercel automatically generates the right build command — you don't need to configure it. If you need a manual override
in `vercel.json`:
...
Vercel auto-sets `npx turbo-ignore` as the "Ignored Build Step" command, which skips builds when a project's
...
- **Using `vercel link` instead of `vercel link --repo`**: Creates `project.json` which only tracks one project. Use
  `--repo` for monorepos.
...
- **Adding a `build` script to package.json for transpilation**: Vercel handles TypeScript compilation. The turbo.json
  `build` task is for orchestration, not transpilation.
