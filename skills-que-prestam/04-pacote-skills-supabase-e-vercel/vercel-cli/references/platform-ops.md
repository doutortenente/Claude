...
Use these commands for account, billing, alerting, and CLI-level operations. Prefer `--format json` where supported for
automation.
...
Billing and purchase commands can change paid account state. Get explicit user confirmation before running mutations
such as `vercel buy ...`.
...
Prefer `VERCEL_TOKEN` for automation. Do not print token values unless the user explicitly needs them and understands
the exposure.
...
`vercel upgrade` changes the installed global CLI. Prefer the project-pinned CLI or package-manager invocation when one
exists, unless the user asked to update a global install.
