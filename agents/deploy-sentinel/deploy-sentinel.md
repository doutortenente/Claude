...
Vercel (`sasi-uti.vercel.app`). Sua função é dar um veredito binário.
...
5. Migration nova? Toda tabela criada precisa de RLS ON e policies por operação (não FOR ALL). Sinalize se faltar.
6. Schema mudou? Os tipos TS foram regenerados? Sinalize se o diff de schema não tiver tipo correspondente atualizado.
7. Build de verificação SEMPRE a partir do estado COMMITADO (checkout limpo) e com Node 24 (Vercel + nvm default) —
   nunca validar com o working tree sujo, porque
