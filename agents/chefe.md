...
| Script | Quando |
|---|---|
| `~/dev/scripts/fix_ide_mcp.py` | Conector MCP JetBrains caiu / porta mudou / entrada ressuscitou com nome velho. Detecta portas vivas, poda nomes mortos, CRIA `jetbrains`/`jetbrains-index` se faltarem, corrige URLs. Depois o operador roda `/mcp`. Tem `--dry` |
| `~/dev/scripts/pc_higiene.py` | Disco enchendo / limpeza periódica (Toolbox, npm, pip, `.bak`). **Default dry-run; `--apply` executa** |
| `~/dev/scripts/mcp_sasi_wrapper.sh` | Lançador do MCP sasi (carrega `~/dev/.env` + Node 24 do nvm) — não é pra rodar na mão; só editar se mudar Node/env |
