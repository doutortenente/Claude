...
6. **Casa única (travada 22-jul-2026): TODO script mora em `~/projetos/scripts/<gaveta>/`.** Nenhum repo tem `scripts/` próprio. Gavetas: `indices/` (build/query dos 3 índices) · `pc/` (limpeza, saúde, MCP da IDE) · `nuvem/` (OneDrive, rclone) · `sasi/` (wrapper MCP, auditoria) · `obsidian/`. Gaveta nova só se o assunto não couber em nenhuma. Exceção: script que É o corpo de uma skill fica em `skills/<nome>/scripts/`. NUNCA deixar script novo em `/tmp` ou `~/Downloads`.
   - **Antes de MOVER qualquer script, procurar quem o chama** (`grep -rn <nome> ~/projetos ~/.claude/memory ~/.config/systemd`). Dois já são cabeados fora do `~/projetos` e quebram calados: systemd `fix-ide-mcp.service` → `scripts/pc/fix_ide_mcp.py` · `.mcp.json` → `scripts/sasi/mcp_sasi_wrapper.sh`.
